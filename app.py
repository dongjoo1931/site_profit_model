...
if not st.session_state.analysis_ready:
    st.markdown(
        """
        <div style="display:flex; align-items:center; justify-content:center; min-height:74vh; text-align:center;">
            <div style="font-size:40px; font-weight:900; color:#1f3a5f; line-height:1.55;">
...
st.markdown(
    """
    <div class="hero">
        <div class="hero-title">분석 결과</div>
        <div class="hero-sub">
            입력한 부지·건물·모듈·비용 조건을 기준으로 운송 가능성, 양중 검토, 구조방식, RC 대비 OSC 비용을 종합 분석한 결과입니다.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with tab1:
    ...
    st.markdown("<div class='section-desc'>현재 입력된 조건을 요약해 전체 프로젝트 개요를 빠르게 확인할 수 있도록 구성했습니다.</div>", unsafe_allow_html=True)
...
with tab3:
    ...
    st.dataframe(pd.DataFrame(crane_results), use_container_width=True, hide_index=True)
    st.markdown("</div>", unsafe_allow_html=True)
