...
st.markdown(
    '<div class="sub-title">현장 접근성, 운송, 양중, 적치, 접합, 공장제작률, 공기단축, 소규모 사업 규모의 경제 미달 위험까지 반영하는 의사결정용 Streamlit 프로토타입입니다.</div>',
    unsafe_allow_html=True,
)

st.markdown(
    '<div class="section-note"><b>주의</b><br>이 버전은 Step 1 판단용 프로토타입입니다. PDF에 있는 정성/정량 규칙은 최대한 반영했지만, 실제 입찰/실행 수준으로 쓰려면 <b>크레인 Load Chart 원본</b>, <b>실제 운송 견적</b>, <b>모듈 공장 견적</b>, <b>RC 공사비 내역서</b>, <b>현장별 장비 반입 동선도</b>를 추가해야 합니다.</div>',
    unsafe_allow_html=True,
)
...
with col_b:
    ...
    project_note = st.text_input("프로젝트 메모", placeholder="예: 협소 도심부지, 청년주택, 중층 계획")
...
if site_address.strip():
    if render_simple_map(site_address.strip()):
        st.caption("주소가 맞으면 아래 상세 조건 입력을 진행하세요.")
else:
...
st.caption(f"선택 모듈 설명: {module_desc}")
st.markdown('</div>', unsafe_allow_html=True)
...
if run_clicked:
    ...
    st.markdown('<div class="result-card">', unsafe_allow_html=True)
    st.markdown("### I. 참고용 RC 장비비 입력 보조")
    ...
    st.markdown('<div class="result-card">', unsafe_allow_html=True)
    st.markdown("### J. 추가 검토 필요 항목")
    additional_checks = [
        "실제 크레인 Load Chart 원본을 반영한 반경별 허용하중 검증",
        "현장 진입로 회전반경 시뮬레이션 및 경찰/도로관리청 협의 필요 여부 확인",
        "모듈 공장 견적서 기준 운송비·설치비·접합비 실측치 반영",
        "RC 기준 공사비를 실제 내역서 또는 표준시장단가로 교체",
        "트레일러 축하중, 총중량, 관리도로 예외(높이 4.2m) 적용 여부 검토",
        "JIT 설치 시 시간대별 도로 통제와 적치 불가 리스크 반영",
    ]
